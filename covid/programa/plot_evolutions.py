...
def _plot_evolution(plot_path, evolutions, by_week, rate_by_100k,
    ...
    for idx, param in enumerate(config.ORIG_COUNT_COLS):
        ...
        if reference_evolutions is not None:
            reference_evolution = reference_evolutions[param]
...
def plot_evolution_per_community(sex=None, date_range=None,
                                 by_week=False,
                                 plot_spain_reference=True):

    _plot_evolution_per_region(by=config.COMMUNITY,
                               sex=sex,
                               date_range=date_range,
                               by_week=by_week,
...
def plot_evolution_per_province(sex=None, date_range=None,
                                by_week=False,
                                plot_spain_reference=True):
    _plot_evolution_per_region(by=config.PROVINCE,
                               sex=sex,
                               date_range=date_range,
                               by_week=by_week,
                               plot_spain_reference=plot_spain_reference)
...
if __name__ == '__main__':
    plot_evolution_spain(by_week=True)
    plot_evolution_per_community(by_week=True)
    plot_evolution_per_province(by_week=True)
